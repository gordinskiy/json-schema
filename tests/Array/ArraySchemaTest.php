...
final class ArraySchemaTest extends TestCase
{
    ...
    public function test_negative_min_contains(): void
    {
        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Array minContains constrain value must be a non-negative number.');

        new ArraySchema(minContains: -1);
    }

    public function test_negative_max_contains(): void
    {
        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Array maxContains constrain value must be a non-negative number.');

        new ArraySchema(maxContains: -1);
    }

    public function test_min_contains_greater_than_max_contains(): void
    {
        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Array minContains constrain cant be greater than maxContains constrain.');

        new ArraySchema(minContains: 10, maxContains: 2);
    }

    public function test_negative_min_items(): void
    {
        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Array minItems constrain value must be a non-negative number.');

        new ArraySchema(minItems: -1);
    }

    public function test_negative_max_items(): void
    {
        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Array maxItems constrain value must be a non-negative number.');

        new ArraySchema(maxItems: -1);
    }

    public function test_min_items_greater_than_max_items(): void
    {
        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Array minItems constrain cant be greater than maxItems constrain.');

        new ArraySchema(minItems: 10, maxItems: 2);
    }
}
